...
import {uploadData, getPageById} from "../actions/DownloadUploadAction";
...
class CreatePage extends BasePage {
    ...
    componentWillMount() {
        if ("id" in this.props.match.params) {
            this.props.getPageById(
                this.props.match.params.id,
                (page)=>{this.render();console.log(page);});
        }
    }
        ...
        return (
            <React.Fragment>
                        ...
                        <Grid item xs={9}>
                            <div style={{verticalAlign: "top"}}>
                                {this.props.siteContent.all.En.map((content, index) => {
                                    ...
                                    const data = {
                                        En: this.props.siteContent.all.En[index],
                                        Fa: this.props.siteContent.all.Fa[index],
                                        bcolor: this.props.siteContent.all.bcolor[index]
                                    };
                                    switch (content.type) {
                                        case "PartModule":
                                            insideModule = <PartModule data={data} key={index}
                                                                       onDelete={this.handleDeleteModule(index)}
                                                                       onAccept={this.handleAccept(index)}
                                                                       index={index}/>
                                            break;
                                        case "OneSectionModule":
                                            insideModule =
                                                <OneSectionModule data={data} onAccept={this.handleAccept(index)}
                                                                  index={index}/>
                                            break;
                                        case "PostModule":
                                            insideModule = <PostModule data={data} key={index}
                                                                       onDelete={this.handleDeleteModule(index)}
                                                                       onAccept={this.handleAccept(index)}
                                                                       index={index}/>
                                            break;
                                        case "PeopleModule":
                                            insideModule = <PeopleModule data={data} key={index}
                                                                         onDelete={this.handleDeleteModule(index)}
                                                                         onAccept={this.handleAccept(index)}
                                                                         index={index}/>
                                            break;
                                        case "TwoSectionModule":
                                            insideModule = <TwoSectionModule data={data} key={index}
                                                                             onDelete={this.handleDeleteModule(index)}
                                                                             onAccept={this.handleAccept(index)}
                                                                             index={index}/>
                                            break;
                                        case "TwoSectionBigModule":
                                            insideModule =
                                                <TwoSectionModuleBig data={data} key={index} content={content}
                                                                     onDelete={this.handleDeleteModule(index)}
                                                                     onAccept={this.handleAccept(index)} index={index}/>
                                            break;
                                    }
                ...
                <YesNoModal open={this.state.openModal}
                            ...
                            handleYesClicked={this.uploadPage}
...
export default connect(mapStateToProps,
    {changeSiteContent, setPageName, uploadData, getPageById})
